...
To install the chart with the release name `choerodon-message`:

```console
$ helm repo add c7n https://openchart.choerodon.com.cn/choerodon/c7n
$ helm repo update
$ helm install choerodon-message c7n/choerodon-message
```
...
| containerPort.actuatorPort | int | `8121` | server management port |
| containerPort.serverPort | int | `8120` | server port |
...
| extraEnv.HZERO_SMS_FAKE | bool | `false` |  |
| extraEnv.HZERO_WEBSOCKET_OAUTHURL | string | `"http://choerodon-oauth/oauth/api/user"` |  |
...
| extraEnv.SPRING_DATASOURCE_URL | string | `"jdbc:mysql://localhost:3306/hzero_message?useUnicode=true&characterEncoding=utf-8&useSSL=false&useInformationSchema=true&remarks=true&serverTimezone=Asia/Shanghai"` |  |
...
| service.nodePort | object | `{"actuator":30127,"server":30126}` | Specify the nodePort value for the LoadBalancer and NodePort service types. |
| service.port | object | `{"actuator":8121,"server":8120}` | server Service port |
